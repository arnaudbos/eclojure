...
(ns stm-io-test-helper
  (:use clojure.test stm.io)
  (:refer-clojure :exclude [dosync]))

;; Small fixture that just exposes a ref
(def stm-io-test-ref (ref 0))
(defn test-ref-fixture
  "Fixture for running a test with a single ref rest each run"
  [func]
  (dosync
    (ref-set stm-io-test-ref 0))
    (func))

;; Refs needed for dosync-fixture, global vars is simpler then local vars
(def stm-io-deref-ref (ref 0))
(def stm-io-commute-ref (ref 0))
(def stm-io-alter-ref (ref 0))
(def stm-io-ref-set-ref (ref 0))
(def stm-io-ensure-ref (ref 0))
...
(defn dosync-fixture
  ...
  (let [lockbject (Object.)
        thread (Thread.
                #(dosync
                   (deref stm-io-deref-ref)
                   (commute stm-io-commute-ref identity)
                   (alter stm-io-alter-ref identity)
                   (ref-set stm-io-alter-ref 0)
                   (ensure stm-io-ensure-ref)
